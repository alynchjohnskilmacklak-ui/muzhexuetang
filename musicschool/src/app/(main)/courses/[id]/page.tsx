...
import { Button, Card, Col, Empty, InputNumber, message, Modal, Popconfirm, Progress, Row, Select, Space, Spin, Statistic, Table, Tag } from 'antd'
...
export default function CourseGroupDetailPage() {
  ...
  const { data: studentsData } = useSWR(enrollOpen ? '/api/students?limit=500' : null, fetcher)
  ...
  const filteredAvailableStudents = studentGrade ? availableStudents.filter((student: Record<string, unknown>) => student.grade === studentGrade) : availableStudents

...
export async function GET() {
  ...
  try {
    ...
    await prisma.user.upsert({ where: { email: 'admin@test.com' }, update: {}, create: { email: 'admin@test.com', password: hash, name: '管理员', role: 'admin' } })
    await prisma.user.upsert({ where: { email: 'parent1@test.com' }, update: {}, create: { email: 'parent1@test.com', password: hash, name: '张爸爸', role: 'parent' } })
    await prisma.user.upsert({ where: { email: 'parent2@test.com' }, update: {}, create: { email: 'parent2@test.com', password: hash, name: '李妈妈', role: 'parent' } })

    const t1 = await prisma.teacher.upsert({ where: { id: 't1' }, update: {}, create: { id: 't1', name: '王老师', phone: '13800001001', subjects: '音乐' } })
    const t2 = await prisma.teacher.upsert({ where: { id: 't2' }, update: {}, create: { id: 't2', name: '李老师', phone: '13800001002', subjects: '数学' } })
    const t3 = await prisma.teacher.upsert({ where: { id: 't3' }, update: {}, create: { id: 't3', name: '张老师', phone: '13800001003', subjects: '英语' } })
    const t4 = await prisma.teacher.upsert({ where: { id: 't4' }, update: {}, create: { id: 't4', name: '赵老师', phone: '13800001004', subjects: '编程' } })
    const t5 = await prisma.teacher.upsert({ where: { id: 't5' }, update: {}, create: { id: 't5', name: '陈老师', phone: '13800001005', subjects: '美术' } })

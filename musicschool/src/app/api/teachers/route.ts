...
export async function POST(req: NextRequest) {
  try {
    ...
    // Auto-create teacher login account: email = pinyin@tea.com, password = pinyin
    const teacherPinyin = chineseToPinyin(teacher.name)
    const teacherEmail = `${teacherPinyin}@tea.com`
    await prisma.user.upsert({
      where: { email: teacherEmail },
      update: { name: teacher.name, password: teacherPinyin },
      create: {
        email: teacherEmail,
        password: teacherPinyin,
        name: teacher.name,
        role: 'teacher',
        status: 'active',
      },

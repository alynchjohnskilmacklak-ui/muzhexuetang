...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Validate one-on-one student count
    ...
    if (classType === 'SMALL_CLASS') {
      ...
      if (roomId) {
        const room = await prisma.room.findUnique({ where: { id: roomId }, select: { capacity: true } })
        if (room && studentIds.length > room.capacity) {
          return NextResponse.json({ error: `学生人数(${studentIds.length})超过教室容量(${room.capacity})` }, { status: 400 })
        }
      }
    }

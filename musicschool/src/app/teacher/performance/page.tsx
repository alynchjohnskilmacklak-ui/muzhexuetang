...
export default function TeacherPerformancePage() {
  ...
  const publish = async () => {
    ...
    setPosting(false)
    if (!res.ok) { message.error(payload.error || '发布失败'); return }
    message.success(`表现反馈已发布 · 推送给 ${selectedStudents.length} 位家长`)
  ...
  const submitReply = async (postId: string) => {
    ...
    if (!res.ok) { message.error('回复失败'); return }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(420px, 480px) minmax(0, 1fr)', gap: 16, alignItems: 'start' }}>
        <Card title="发布在校表现" bordered={false} style={{ borderRadius: 10 }}>
          <Space direction="vertical" size={14} style={{ width: '100%' }}>
              ...
              <Space direction="vertical" style={{ width: '100%', marginTop: 8 }}>
                {RATING_KEYS.map(item => (
                  <div key={item.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Rate value={ratings[item.key]} onChange={v => setRatings(c => ({ ...c, [item.key]: v }))} style={{ color: '#f5a623' }} />
                  </div>
                ))}
              </Space>
            </div>
            ...
            <Button type="primary" icon={<SendOutlined />} loading={posting} onClick={publish}
              style={{ background: '#E8784A' }} block
              disabled={selectedStudents.length === 0}>
              发布反馈 · 推送给 {selectedStudents.length} 位家长
            </Button>
          </Space>
        </Card>
                ...
                <List.Item style={{ display: 'block', borderBottom: '1px solid #f0e7de' }}>
                  ...
                  {replyingId === post.id && (
                    <Space.Compact style={{ width: '100%', marginTop: 8 }}>
                      ...
                      <Button type="primary" onClick={() => submitReply(post.id)}>发送</Button>
                    </Space.Compact>
                  )}
                </List.Item>
              )

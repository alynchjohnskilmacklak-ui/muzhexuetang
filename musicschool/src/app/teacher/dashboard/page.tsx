...
export default function TeacherDashboardPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <section style={{ background: '#123326', borderRadius: 12, padding: '26px 30px', color: '#fff' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'stretch', gap: 20, flexWrap: 'wrap' }}>
          <div style={{ minWidth: 260, flex: 1 }}>
            ...
            <h1 style={{ margin: '8px 0 8px', fontSize: 24, lineHeight: 1.3, letterSpacing: 0 }}>{teacher.name}老师，{getGreeting()}</h1>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(96px, 1fr))', gap: 10, flex: '1 1 460px' }}>
            {heroItems.map((item) => (
          ...
          <Card bordered={false} title="今日待办" style={{ borderRadius: 12, marginBottom: 16 }}>
            {todos.length ? (
              <List
                ...
                renderItem={(item) => (
                  <List.Item
                    actions={[<Button key="action" size="small" type="link" onClick={() => router.push(item.href)}>{item.actionLabel}</Button>]}
              ...
              <List
                ...
                renderItem={(lesson) => (
                  <List.Item style={{ padding: '14px 0' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '92px 1fr auto', width: '100%', gap: 14, alignItems: 'center' }}>
                      ...
                      <Space wrap>
                        ...
                        <Button size="small" onClick={() => router.push('/teacher/preparation')}>备课</Button>
                      </Space>
                    </div>
                  </List.Item>
                )}
          ...
          <Card bordered={false} title="反馈与试卷待办" style={{ borderRadius: 12, marginBottom: 16 }}>
            {feedbackTasks.length ? (
              <List
                ...
                renderItem={(item) => (
                  <List.Item actions={[<Button key="action" size="small" type="link" onClick={() => router.push(item.href)}>{item.actionLabel}</Button>]}>
                    <List.Item.Meta

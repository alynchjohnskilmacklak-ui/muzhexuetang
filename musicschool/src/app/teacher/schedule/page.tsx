    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => setWeekOffset(0)} style={navBtnStyle}>本周</button>
          <button onClick={() => setWeekOffset(o => o + 1)} style={navBtnStyle}>→</button>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', border: '0.5px solid var(--color-border, #EEE7E1)', borderRadius: 8, overflow: 'hidden', width: 'fit-content', marginBottom: 12 }}>
        <button onClick={() => setScheduleType('group')} style={{
          padding: '7px 18px', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: scheduleType === 'group' ? 500 : 400,
        ...
        <button onClick={() => setScheduleType('intensive')} style={{
          padding: '7px 18px', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: scheduleType === 'intensive' ? 500 : 400,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 16 }}>
        <Card bordered={false} style={{ borderRadius: 10 }}><Text type="secondary">本周课次</Text><div style={{ fontSize: 24, fontWeight: 700 }}>{stats.total}</div></Card>
        <Card bordered={false} style={{ borderRadius: 10 }}><Text type="secondary">已完成</Text><div style={{ fontSize: 24, fontWeight: 700, color: '#1D9E75' }}>{stats.completed}</div></Card>
        <Card bordered={false} style={{ borderRadius: 10 }}><Text type="secondary">在带学员</Text><div style={{ fontSize: 24, fontWeight: 700, color: '#E8784A' }}>{stats.students}</div></Card>
      </div>

...
export default function TeacherAttendancePage() {
  ...
  const submitAttendance = async () => {
    ...
    if (payload.alreadyDeducted) {
      message.success(payload.message || `考勤已更新，未重复扣课时`)
    } else {
      message.success(`✅ ${payload.message || '考勤已提交'}`, 4)
    }
  }
        ...
        <Card bordered={false} style={{ borderRadius: 10 }}
          ...
          extra={selectedLesson ? (
            <Space>
              ...
              <Button type="primary" loading={submitting} onClick={submitAttendance} style={{ background: '#E8784A' }}>提交考勤</Button>
            </Space>
          ) : null}>
          {!selectedLesson ? <Empty description="请从左侧选择课次" /> : (
            <>
              {/* Progress bar */}
              <div style={{ background: 'var(--color-background-secondary, #faf8f5)', borderRadius: 8, padding: '8px 12px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                {students.map((s: any) => {
                  ...
                  return (
                    <div key={s.studentId} onClick={() => cycleAttendance(s.studentId)} style={{
                      borderRadius: 9, padding: '10px 8px', textAlign: 'center', cursor: 'pointer', userSelect: 'none',

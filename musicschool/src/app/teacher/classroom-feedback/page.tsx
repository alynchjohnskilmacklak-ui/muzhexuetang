...
function SortableHomework({ item, onChange, onRemove }: { item: HomeworkItem; onChange: (value: string) => void; onRemove: () => void }) {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({ id: item.id })
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, display: 'grid', gridTemplateColumns: '28px 1fr 36px', gap: 8, alignItems: 'center' }}>
      <Button size="small" icon={<HolderOutlined />} {...attributes} {...listeners} />
      <Input value={item.content} onChange={(event) => onChange(event.target.value)} placeholder={`作业${item.order}描述`} />
...
export default function ClassroomFeedbackPage() {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(520px, 1fr) 360px', gap: 16, alignItems: 'start' }}>
        <Space direction="vertical" size={16} style={{ width: '100%' }}>
          ...
          <Card title="上传资料" bordered={false} style={{ borderRadius: 10 }}>
            <Dragger beforeUpload={uploadFile} accept="image/*,.pdf" multiple maxCount={9} showUploadList={false}>
            ...
            {students.length ? (
              <List dataSource={students} renderItem={(student: any) => (
                <List.Item>
                  ...
                  <Rate value={ratings[student.id] || 5} onChange={(value) => setRatings((current) => ({ ...current, [student.id]: value }))} />
                </List.Item>
              )} />
          ...
          <Card bordered={false} style={{ borderRadius: 10 }}>
            <Space>
              <Button icon={<SaveOutlined />} loading={saving} onClick={() => save('DRAFT')}>保存草稿</Button>
              <Button type="primary" icon={<SendOutlined />} loading={saving} onClick={() => save('PUBLISHED')} style={{ background: '#E8784A' }}>发布 · 同步家长</Button>
            </Space>
          </Card>
        </Space>

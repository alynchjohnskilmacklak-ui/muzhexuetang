...
export async function GET() {
  ...
  try {
    ...
    const s1 = await prisma.student.upsert({ where: { id: 's1' }, update: {}, create: { id: 's1', name: '张三', status: 'active', parentId: p1!.id } })
    const s2 = await prisma.student.upsert({ where: { id: 's2' }, update: {}, create: { id: 's2', name: '李四', status: 'active', parentId: p2!.id } })
    const s3 = await prisma.student.upsert({ where: { id: 's3' }, update: {}, create: { id: 's3', name: '王五', status: 'active', parentId: p1!.id } })
    const s4 = await prisma.student.upsert({ where: { id: 's4' }, update: {}, create: { id: 's4', name: '赵六', status: 'active', parentId: p2!.id } })
    const s5 = await prisma.student.upsert({ where: { id: 's5' }, update: {}, create: { id: 's5', name: '孙七', status: 'active', parentId: p1!.id } })
    ...
    await prisma.fee.createMany({ data: [
      { studentId: s1.id, courseId: c1.id, amount: 3800, type: '课时费', status: 'paid', paidAt: new Date() },
      { studentId: s2.id, courseId: c2.id, amount: 2800, type: '课时费', status: 'paid', paidAt: new Date() },
      { studentId: s3.id, courseId: c3.id, amount: 3200, type: '课时费', status: 'paid', paidAt: new Date() },
      { studentId: s4.id, courseId: c4.id, amount: 4200, type: '课时费', status: 'paid', paidAt: new Date() },
    ]})

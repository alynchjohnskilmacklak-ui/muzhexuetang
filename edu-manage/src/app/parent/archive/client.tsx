...
import {
  ...
  DashboardOutlined, ExpandOutlined, CompressOutlined,
...
export function ParentArchiveClient({ initial }: { initial: InitialData }) {
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 4px' }}>
      ...
      {loadingMask}
      {!profile && !isLoading && <Card bordered={false} style={{ borderRadius: 14, textAlign: 'center', minHeight: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Empty description="该学员暂无档案数据" image={Empty.PRESENTED_IMAGE_SIMPLE} /></Card>}
      ...
      {profile && (<>
        ...
        <Card bordered={false} style={{ borderRadius: 14, marginBottom: 12, border: '1px solid rgba(0,0,0,.06)', padding: '10px 0' }}>
          ...
          {profile.record.timeline[0] && (
            <Paragraph type="secondary" style={{ marginTop: 8, marginBottom: 0, fontSize: 12, maxWidth: 600 }} ellipsis={{ rows: 1 }}>
        ...
        <Collapse ghost size="small" activeKey={[...(studyOpen ? ['study'] : []), ...(habitOpen ? ['habit'] : [])]} onChange={k => { setStudyOpen(k.includes('study')); setHabitOpen(k.includes('habit')) }}
          items={[
            {
              key: 'study', label: (
                <span><BookOutlined style={{ color: '#1D9E75', marginRight: 6 }} />学 · 知识掌握 {profile.study.mastery.total > 0 ? `· 已掌握 ${profile.study.mastery.masteredPct}%` : ''}</span>
              ),
              children: (
                <div>
                  ...
                  ) : <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 8 }}>暂无知识掌握数据</Text>}
                  {profile.study.weaknesses.length > 0 && (
                    <div style={{ marginBottom: 8 }}>
                      <Text strong style={{ fontSize: 12, display: 'block', marginBottom: 4 }}>薄弱知识点</Text>
            ...
            {
              key: 'habit', label: (
                <span><DashboardOutlined style={{ color: '#E8784A', marginRight: 6 }} />习 · 课堂状态 {profile.habits.attendanceRate !== null ? `· 出勤 ${profile.habits.attendanceRate}%` : ''}</span>
              ),
              children: (
                <div>
                  ...
                  ) : <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 8 }}>暂无课堂情绪记录</Text>}
        ...
        <div style={{ marginTop: 16 }}>
          ...
          {profile.record.timeline.filter(item => {
            if (!timeFilter) return true
            const chip = FILTER_CHIPS.find(c => c.key === timeFilter)
            return chip?.types?.includes(item.type)
          }).length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
              {profile.record.timeline.filter(item => !timeFilter || FILTER_CHIPS.find(c => c.key === timeFilter)?.types?.includes(item.type)).map((item, i) => (
                <div key={`${item.type}-${i}`} onClick={() => handleTimelineClick(item)} style={{
                  display: 'flex', gap: 10, padding: '10px 0', borderBottom: '1px solid rgba(0,0,0,.04)', fontSize: 13,
                  cursor: (item.images?.length || item.refType) ? 'pointer' : 'default',
                }}>
                  <span style={{ flexShrink: 0, width: 28, height: 28, borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', background: `${TYPE_COLOR[item.type]}15`, color: TYPE_COLOR[item.type], fontSize: 14 }}>{TYPE_ICON[item.type] || <RiseOutlined />}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {TIMELINE_SOURCE[item.type] && (
                      <span style={{ fontSize: 10, color: TIMELINE_SOURCE[item.type].color, background: `${TIMELINE_SOURCE[item.type].color}14`, padding: '1px 7px', borderRadius: 9999, marginLeft: 6 }}>
        ...
        {profile.record.trendBySubject.length > 0 && (
          <Card bordered={false} style={{ borderRadius: 14, marginTop: 16, border: '1px solid rgba(0,0,0,.06)' }}>
            <Text strong style={{ fontSize: 14, display: 'block', marginBottom: 10 }}><RiseOutlined style={{ color: '#534AB7', marginRight: 6 }} />档 · 成绩趋势</Text>
        ...
        {/* ── 案：目标 + 寄语 ── */}
        <Card bordered={false} style={{ borderRadius: 14, marginTop: 16, border: '1px solid rgba(0,0,0,.06)' }}>
          <Text strong style={{ fontSize: 14, display: 'block', marginBottom: 10 }}><FlagOutlined style={{ color: '#EF9F27', marginRight: 6 }} />案 · 学习目标与教师寄语</Text>
          ...
          ) : <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 12 }}>暂无学习目标</Text>}
          <div style={{ padding: '10px 14px', background: '#faf8f5', borderRadius: 12, border: '1px solid rgba(0,0,0,.04)' }}>
            ...
            {profile.profileCase.teacherSummary ? (
              <div>
                <Tag color="purple" style={{ borderRadius: 9999, marginBottom: 6, fontSize: 10 }}>
                  {profile.profileCase.teacherSummary.teacherName || '老师'} · {format(new Date(profile.profileCase.teacherSummary.periodStart), 'M/d')} 至 {format(new Date(profile.profileCase.teacherSummary.periodEnd), 'M/d')}
                </Tag>
                ...
                {profile.profileCase.teacherSummary.suggestions && (
                  <Paragraph type="secondary" style={{ fontSize: 12, margin: '6px 0 0', whiteSpace: 'pre-wrap' }}>下一步建议：{profile.profileCase.teacherSummary.suggestions}</Paragraph>
                )}
              </div>
      ...
      <Modal open={!!detailItem} onCancel={() => setDetailItem(null)} footer={null} width={560} title={detailItem?.title || '详情'}>
        {detailItem && (
          <div>
            ...
            {detailItem.refType === 'paper' && detailItem.refId && (
              <Button type="link" onClick={() => { setDetailItem(null); router.push(`/parent/archive?paperId=${detailItem.refId}`) }} style={{ padding: 0, marginTop: 8 }}>查看试卷详情</Button>
            )}
            {detailItem.refType === 'post' && detailItem.refId && (
              <Button type="link" onClick={() => { setDetailItem(null); router.push(`/parent/growth?postId=${detailItem.refId}`) }} style={{ padding: 0, marginTop: 8 }}>查看成长动态详情</Button>
            )}
          </div>
        )}
      </Modal>
      ...
      {/* ── B4 Modal: 学情报告 ── */}
      <Modal open={reportOpen} onCancel={() => setReportOpen(false)} footer={
        <Button onClick={() => setReportOpen(false)}>关闭</Button>
      } width={600} title="本期学情报告">
        {profile && (
          <div>
            <div style={{ marginBottom: 16 }}><Text strong style={{ fontSize: 15 }}>{profile.identity.name} · 学情报告</Text></div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}>
              {[
                { label: '出勤率', value: profile.overview.attendanceRate !== null ? `${profile.overview.attendanceRate}%` : '—' },
                { label: '知识掌握', value: profile.study.mastery.total > 0 ? `${profile.study.mastery.masteredPct}%` : '—' },
                { label: '本期试卷', value: `${profile.overview.paperCount} 份` },
                { label: '获得徽章', value: `${profile.overview.badgeCount} 枚` },
              ].map(item => (
                <Card key={item.label} size="small" bordered={false} style={{ borderRadius: 10, background: '#FFFBF7', textAlign: 'center' }}>
                  <Text type="secondary" style={{ fontSize: 11 }}>{item.label}</Text>
                  <div><Text strong style={{ fontSize: 18, color: '#E8784A' }}>{item.value}</Text></div>
                </Card>
              ))}
            </div>
            {profile.study.weaknesses.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <Text strong style={{ fontSize: 13, display: 'block', marginBottom: 4 }}>薄弱知识点</Text>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>{profile.study.weaknesses.map((w, i) => <Tag key={i} color="red" style={{ borderRadius: 9999, fontSize: 11 }}>{w.topic}</Tag>)}</div>
              </div>
            )}
            {profile.profileCase.teacherSummary && (
              <div style={{ marginBottom: 12, padding: '10px 14px', background: '#faf8f5', borderRadius: 10, border: '1px solid rgba(0,0,0,.04)' }}>
                <Text strong style={{ fontSize: 13, display: 'block', marginBottom: 4 }}>教师寄语</Text>
                <Paragraph type="secondary" style={{ fontSize: 13, margin: 0, whiteSpace: 'pre-wrap' }}>{profile.profileCase.teacherSummary.summary}</Paragraph>
                {profile.profileCase.teacherSummary.suggestions && <Paragraph type="secondary" style={{ fontSize: 13, margin: '6px 0 0', whiteSpace: 'pre-wrap' }}>建议：{profile.profileCase.teacherSummary.suggestions}</Paragraph>}
              </div>
            )}
            <div style={{ marginTop: 8, textAlign: 'center' }}>
              <Text type="secondary" style={{ fontSize: 11 }}>PDF 导出即将上线</Text>
            </div>
          </div>
        )}
      </Modal>

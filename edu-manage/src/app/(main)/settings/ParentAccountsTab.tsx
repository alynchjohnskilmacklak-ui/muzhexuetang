...
export function ParentAccountsTab() {
  const { data, isLoading, mutate } = useSWR<AccountsResponse>('/api/settings/parent-accounts', fetcher)
  ...
  return (
    <Card bordered={false} style={{ borderRadius: 10 }}>
      <Space direction="vertical" size={12} style={{ width: '100%' }}>
        <Typography.Title level={4} style={{ margin: 0 }}>家长登录账号</Typography.Title>
        <Alert
          type="info"
          message="家长账号在绑定学员时自动创建，停用后家长将无法登录查看子女信息。"
